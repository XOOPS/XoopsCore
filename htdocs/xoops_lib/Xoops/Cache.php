...
/**
 * Xoops_Cache provides a consistent interface to Caching in your application. It allows you
 * to use several different Cache engines, without coupling your application to a specific
 * implementation.  It also allows you to change out cache storage or configuration without effecting
 * the rest of your application.
 * You can configure Cache engines in your application's file.  A sample configuration would
 * be
 * {{{
 *    Xoops_Cache::config('shared', array(
 *        'engine' => 'Apc',
 *        'prefix' => 'my_app_'
 *  ));
 * }}}
 ...
 * general all Cache operations are supported by all cache engines.  However, Xoops_Cache::increment() and
 * Xoops_Cache::decrement() are not supported by File caching.
 *
 * @package       Cake.Cache
 */
class Xoops_Cache
{
     ...
     * `Xoops_Cache::config('my_config', array('engine' => 'File', 'path' => TMP));`
     ...
     * - `extension` Used by FileCache.  Appends to file name
     *
     *
    ...
    protected static function _buildEngine($name)
    {
        ...
        $cacheClass = 'Xoops_Cache_' . ucfirst($config['engine']);
     ...
     * `Xoops_Cache::set(array('duration' => '+30 minutes'), 'my_config');`
     * Or to set one value.
     * `Xoops_Cache::set('duration', '+30 minutes', 'my_config');`
     * To reset a config back to the originally configured values.
     * `Xoops_Cache::set(null, 'my_config');`
     *
     ...
     * `Xoops_Cache::write('cached_data', $data);`
     * Writing to a specific cache config:
     * `Xoops_Cache::write('cached_data', $data, 'long_term');`
     *
    ...
    public static function write($key, $value, $config = 'default')
    {
        ...
        if ($success === false && $value !== '') {
            trigger_error(sprintf("%s cache was unable to write '%s' to %s cache", $config, $key, self::$_engines[$config]->settings['engine']), E_USER_WARNING);
        }
        return $success;
    }
     ...
     * `Xoops_Cache::read('my_data');`
     * Reading from a specific cache configuration.
     * `Xoops_Cache::read('my_data', 'long_term');`
     *
     * @param string $key    Identifier for the data
     ...
     * `Xoops_Cache::delete('my_data');`
     * Deleting from a specific cache configuration.
     * `Xoops_Cache::delete('my_data', 'long_term');`
     *
     * @param string $key    Identifier for the data
