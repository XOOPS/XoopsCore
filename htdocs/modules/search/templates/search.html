<fieldset>
    <{if $search}>
        ...
        <div>
            ...
            <span class="bold">
            <{foreachq from=$queries item=query name=foo}>
        ...
        <{if count($ignored_queries) != 0}>
            <div>
                <{$ignored_words}>&nbsp;:&nbsp;
                <span class="bold">
                <{foreachq from=$ignored_queries item=query name=foo}>
                    <{$query}><{if !$smarty.foreach.foo.last}>,<{/if}>
                <{/foreach}>
                </span>
            </div>
    ...
    <{if count($modules) > 0}>
        <{foreachq from=$modules item=module name=foo}>
