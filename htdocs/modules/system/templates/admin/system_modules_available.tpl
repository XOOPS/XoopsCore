...
<div class="xo-module rounded">
    ...
    <div class="module_options">
        <a class="xo-tooltip" href="javascript:" onclick="$('.install-{$install->getInfo(mid)}').removeClass('hide'); $('.install-{$install->getInfo(mid)}').toggle('slow');" title="{translate key='DETAILS'}"><span class="ico-magnifier"></span>{translate key='DETAILS'}</a>
