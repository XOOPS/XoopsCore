...
class UserrankRanksForm extends XoopsThemeForm
{
    /**
    ...
    public function __construct(XoopsObject &$obj)
    {
        ...
        $this->addElement( new XoopsFormText( _AM_USERRANK_TITLE, 'rank_title', 4, 50, $obj->getVar('rank_title')), true );
        $min = new XoopsFormText( _AM_USERRANK_MINPOST, 'rank_min', 1, 10, $obj->getVar('rank_min'));
        $min->setPattern('^\d+$',_AM_USERRANK_ERROR_POST);
        $this->addElement($min);
        $max = new XoopsFormText( _AM_USERRANK_MAXPOST, 'rank_max', 1, 10, $obj->getVar('rank_max'));
        $max->setPattern('^\d+$',_AM_USERRANK_ERROR_POST);
        $this->addElement($max);

        $imgtray_img = new XoopsFormElementTray( _AM_USERRANK_IMAGE, '<br />' );
        ...
        $fileseltray_img->addElement( new XoopsFormFile( _AM_USERRANK_UPLOAD , 'rank_image', 500000 ), false );
        ...
        $special_tray = new XoopsFormElementTray(_AM_USERRANK_SPECIAL, '<br />');
        ...
        if ( !$obj->isNew() ) {
            $this->addElement( new XoopsFormHidden( 'rank_id', $obj->getVar('rank_id')) );
        }
        $this->addElement( new XoopsFormHidden( 'op', 'userrank_save') );
