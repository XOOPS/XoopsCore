...
<table class="outer tablesorter">
    ...
    <tbody>
    <{foreach item=file_arr from=$file_arr}>
    <tr class="<{cycle values='even,odd'}> alignmiddle">
        <td class="txtleft">
            <{$file_arr.name}>
        </td>
        ...
        <td class="xo-actions txtcenter span2">
            <a href="<{$xoops_url}>/modules/maintenance/dump/<{$file_arr.name}>" title="<{$smarty.const._AM_MAINTENANCE_DUMP_DOWNLOAD}>">
                <img src="<{xoModuleIcons16 download.png}>" alt="<{$smarty.const._AM_MAINTENANCE_DUMP_DOWNLOAD}>">
            </a>
            <a href="dump.php?op=dump_delete&amp;filename=<{$file_arr.name}>" title="<{$smarty.const._AM_MAINTENANCE_DELETE}>">
