...
if (!empty($_POST['copy']) && !empty($_POST['old_prefix'])) {
    ...
    $new_prefix = empty($_POST['new_prefix']) ? 'x' . mb_substr(md5(time()), -5) : $_POST['new_prefix'];
    ...
    while (false !== ($row_table = $db->fetchArray($srs))) {
        ...
        if (mb_substr($old_table, 0, mb_strlen($old_prefix) + 1) !== $old_prefix . '_') {
            continue;
        }

        $new_table = $new_prefix . mb_substr($old_table, mb_strlen($old_prefix));
    ...
    if (!empty($_POST['backup']) && !empty($_POST['prefix'])) {
        ...
        while (false !== ($row_table = $db->fetchArray($srs))) {
            ...
            if (mb_substr($table, 0, mb_strlen($prefix) + 1) !== $prefix . '_') {
                continue;
            }
            ...
            while ($row = mysql_fetch_row($result)) {
                ...
                for ($j = 0; $j < $fields_cnt; ++$j) {
                    ...
                    // a binary field
                        ...
                        if (mb_stristr($field_flags[$j], 'BINARY') && 'datetime' !== $fields_meta->type && 'date' !== $fields_meta->type && 'time' !== $fields_meta->type && 'timestamp' !== $fields_meta->type
                        ) {
        ...
        header('Content-Length: ' . mb_strlen($export_string));
        set_time_limit(0);
        echo $export_string;
        exit;
        // DROP TABLES
...
$prefixes = [];
while (false !== ($row_table = $db->fetchArray($srs))) {
    if ('_users' === mb_substr($row_table['Name'], -6)) {
        $prefixes[] = [
            'name' => mb_substr($row_table['Name'], 0, -6),
...
foreach ($prefixes as $prefix) {
    ...
    foreach ($tables as $table) {
        ...
        if ($prefix['name'] . '_' === mb_substr($table, 0, mb_strlen($prefix['name']) + 1)) {
            ++$table_count;
        }
    }
    // check if prefix_xoopscomments exists
