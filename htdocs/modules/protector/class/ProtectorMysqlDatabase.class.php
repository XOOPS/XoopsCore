...
class ProtectorMySQLDatabase extends XoopsMySQLDatabaseProxy
{
    ...
    public function separateStringsInSQL($sql)
    {
        ...
        for ($i = 0; $i < $sql_len; ++$i) {
            ...
            if ($in_string) {
                while (1) {
                    $new_i = mb_strpos($sql, $string_start, $i);
                    $current_string .= mb_substr($sql, $i, $new_i - $i + 1);
                    $i = $new_i;
                    if (false === $i) {
                        break 2;
    ...
    public function checkSql($sql)
    {
        ...
        foreach ($this->doubtful_requests as $request) {
            if (addslashes($request) != $request) {
                if (mb_stristr($sql, trim($request))) {
                    // check the request stayed inside of strings as whole
                    $ok_flag = false;
                    foreach ($strings as $string) {
                        if (mb_strstr($string, $request)) {
                            $ok_flag = true;
                            break;
                        }
                    }
        ...
        foreach ($this->doubtful_requests as $request) {
            if (mb_strstr($sql_wo_strings, trim($request))) {
                $this->injectionFound($sql);
            }
        }
    ...
    {
        $sql4check = mb_substr($sql, 7);
        foreach ($this->doubtful_needles as $needle) {
            if (mb_stristr($sql4check, $needle)) {
                $this->checkSql($sql);
                break;
            }
        }
