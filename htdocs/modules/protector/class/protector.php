...
class protector
{
    ...
    public function _initial_recursive($val, $key)
    {
        if (is_array($val)) {
            ...
            if (@$this->_conf['san_nullbyte'] && mb_strstr($val, chr(0))) {
    ...
    {
        return \XoopsBaseConfig::get('trust-path') . '/modules/protector/configs/bwlimit' . mb_substr(md5(\XoopsBaseConfig::get('root-path') . \XoopsBaseConfig::get('db-user') . \XoopsBaseConfig::get('db-prefix')), 0, 6);
    }
    ...
    {
        return \XoopsBaseConfig::get('root-path') . '/modules/protector/configs/badips' . mb_substr(md5(\XoopsBaseConfig::get('root-path') . \XoopsBaseConfig::get('db-user') . \XoopsBaseConfig::get('db-prefix')), 0, 6);
    }
    ...
    {
        return \XoopsBaseConfig::get('var-path') . '/configs/protector_group1ips_' . mb_substr(md5(\XoopsBaseConfig::get('root-path') . \XoopsBaseConfig::get('db-user') . \XoopsBaseConfig::get('db-prefix')), 0, 6);
    }

    public function get_filepath4confighcache()
    {
        return XOOPS_VAR_PATH . '/configs/protector_configcache_' . mb_substr(md5(XOOPS_ROOT_PATH . XOOPS_DB_USER . XOOPS_DB_PREFIX), 0, 6);
    }
    ...
    public function ip_match($ips)
    {
        foreach ($ips as $ip => $info) {
            if ($ip) {
                switch (mb_substr($ip, -1)) {
                    case '.':
                        // foward match
                        if (mb_substr(@$_SERVER['REMOTE_ADDR'], 0, mb_strlen($ip)) == $ip) {
    ...
    public function deny_by_htaccess($ip = null)
    {
        ...
        if (preg_match("/^(.*)#PROTECTOR#\s+(DENY FROM .*)\n#PROTECTOR#\n(.*)$/si", $ht_body, $regs)) {
            if (mb_substr($regs[2], -mb_strlen($ip)) == $ip) {
                return true;
            }
    ...
    public function bigumbrella_outputcheck($s)
    {
        ...
        if (function_exists('headers_list')) {
            foreach (headers_list() as $header) {
                if (mb_stristr($header, 'Content-Type:') && !mb_stristr($header, 'text/html')) {
                    return $s;
                }
            }
        }
    ...
    public function intval_allrequestsendid()
    {
        ...
        foreach ($_GET as $key => $val) {
            if ('id' === mb_substr($key, -2) && !is_array($_GET[$key])) {
        ...
        foreach ($_POST as $key => $val) {
            if ('id' === mb_substr($key, -2) && !is_array($_POST[$key])) {
                $newval = preg_replace('/[^0-9a-zA-Z_-]/', '', $val);
        ...
        foreach ($_COOKIE as $key => $val) {
            if ('id' === mb_substr($key, -2) && !is_array($_COOKIE[$key])) {
                $newval = preg_replace('/[^0-9a-zA-Z_-]/', '', $val);
    ...
    public function eliminate_dotdot()
    {
        ...
        foreach ($_GET as $key => $val) {
            ...
            if ('../' === mb_substr(trim($val), 0, 3) || mb_strstr($val, '../../')) {
    ...
    public function check_uploaded_files()
    {
        ...
        foreach ($_FILES as $_file) {
            ...
            if (!empty($_file['name']) && is_string($_file['name'])) {
                $ext = mb_strtolower(mb_substr(mb_strrchr($_file['name'], '.'), 1));
                if ('jpeg' === $ext) {
                    $ext = 'jpg';
                } else {
                    if ('tiff' === $ext) {
    ...
    public function check_sql_isolatedcommentin($sanitize = true)
    {
        ...
        foreach ($this->_doubtful_requests as $key => $val) {
            $str = $val;
            while ($str = mb_strstr($str, '/*')) {
                $str = mb_strstr(mb_substr($str, 2), '*/');
                if (false === $str) {
    ...
    public function _spam_check_point_recursive($val)
    {
        if (is_array($val)) {
            ...
            foreach (preg_split('#https?\:\/\/#i', $val) as $fragment) {
                if (0 !== strncmp($fragment, $http_host, mb_strlen($http_host))) {
                    ++$count;
                }
            }
            if ($count > 0) {
    ...
    public function disable_features()
    {
        ...
        if ($this->_conf['disable_features'] & 1) {
            ...
            if ( /* ! stristr( $_SERVER['SCRIPT_NAME'] , 'modules' ) && */
                'xmlrpc.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -10)
            ) {
        ...
        if ($this->_conf['disable_features'] & 1024) {
            // root controllers
            if (!mb_stristr(@$_SERVER['SCRIPT_NAME'], 'modules')) {
                // zx 2004/12/13 misc.php debug (file check)
                if ('misc.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -8) && ('debug' === $_GET['type'] || 'debug' === $_POST['type']) && !preg_match('/^dummy_[0-9]+\.html$/', $_GET['file'])) {
                    $this->output_log('misc debug');
                    exit;
                }

                // zx 2004/12/13 misc.php smilies
                if ('misc.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -8) && ('smilies' === $_GET['type'] || 'smilies' === $_POST['type']) && !preg_match('/^[0-9a-z_]*$/i', $_GET['target'])) {
                    $this->output_log('misc smilies');
                    exit;
                }

                // zx 2005/1/5 edituser.php avatarchoose
                if ('edituser.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -12) && 'avatarchoose' === $_POST['op'] && mb_strstr($_POST['user_avatar'], '..')) {
            ...
            if ('modules/system/admin.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -24) && ('findusers' === $_GET['fct'] || 'findusers' === $_POST['fct'])) {
                foreach ($_POST as $key => $val) {
                    if (mb_strstr($key, "'") || mb_strstr($val, "'")) {
            ...
            // news submit.php
            if ('modules/news/submit.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -23) && isset($_POST['preview']) && 0 !== mb_strpos(@$_SERVER['HTTP_REFERER'], \XoopsBaseConfig::get('url') . '/modules/news/submit.php')) {
                $HTTP_POST_VARS['nohtml'] = $_POST['nohtml'] = 1;
            }
            // news admin/index.php
            if ('modules/news/admin/index.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -28) && ('preview' === $_POST['op'] || 'preview' === $_GET['op']) && 0 !== mb_strpos(@$_SERVER['HTTP_REFERER'], \XoopsBaseConfig::get('url') . '/modules/news/admin/index.php')) {
                $HTTP_POST_VARS['nohtml'] = $_POST['nohtml'] = 1;
            }
            // comment comment_post.php
            if (isset($_POST['com_dopreview']) && !mb_strstr(mb_substr(@$_SERVER['HTTP_REFERER'], -16), 'comment_post.php')) {
                $HTTP_POST_VARS['dohtml'] = $_POST['dohtml'] = 0;
            }
            // disable preview of system's blocksadmin
            if ('modules/system/admin.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -24) && ('blocksadmin' === $_GET['fct'] || 'blocksadmin' === $_POST['fct']) && isset($_POST['previewblock']) /* && strpos( $_SERVER['HTTP_REFERER'] , \XoopsBaseConfig::get('url').'/modules/system/admin.php' ) !== 0 */) {
                die("Danger! don't use this preview. Use 'altsys module' instead.(by Protector)");
            }
            // tpl preview
            if ('modules/system/admin.php' === mb_substr(@$_SERVER['SCRIPT_NAME'], -24) && ('tplsets' === $_GET['fct'] || 'tplsets' === $_POST['fct'])) {
