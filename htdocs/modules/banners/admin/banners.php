...
$mimetypes = ['image/gif', 'image/jpeg', 'image/pjpeg', 'image/x-png', 'image/png', 'application/x-shockwave-flash'];
$upload_size = 500000;
// Get Action type
    ...
    default:
        ...
        if ($banner_finish_count > 0) {
            foreach (array_keys($banner_finish_arr) as $i) {
                ...
                if (0 != $impressions) {
                    $percent = mb_substr(100 * $clicks / $impressions, 0, 5);
                } else {
                    $percent = 0;
                }
                $img = '';
    ...
    case 'delete':
        ...
        if ($bid > 0) {
            ...
            if (isset($_POST['ok']) && 1 == $_POST['ok']) {
                ...
                $namefile = substr_replace($obj->getVar('imageurl'), '', 0, mb_strlen($xoops_url . '/uploads/banners/'));
                ...
                if ($obj->getVar('banner_htmlbanner')) {
                    ...
                    if ('.swf' === mb_strtolower(mb_substr($imageurl, mb_strrpos($imageurl, '.')))) {
