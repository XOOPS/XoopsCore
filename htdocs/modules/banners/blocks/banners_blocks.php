...
function banners_blocks_edit($options)
{
    ...
    switch ($options[0]) {
        case 'random':
            ...
            $form .= '<option value="0" ' . (false === array_search(0, $options) ? '' : 'selected="selected"') . '>' . _MB_BANNERS_ALLCLIENTS . "</option>\n";
            foreach (array_keys($client_arr) as $i) {
                $form .= '<option value="' . $client_arr[$i]->getVar('cid') . '" ' . (false === array_search($client_arr[$i]->getVar('cid'), $options) ? '' : 'selected="selected"') . '>' . $client_arr[$i]->getVar('name') . "</option>\n";
            }
            $form .= "</select>\n";
            break;
        case 'id':
