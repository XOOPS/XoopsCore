...
<table id="xo-bannersclient-sorter" cellspacing="1" class="outer tablesorter">
    ...
    <tbody>
    <{foreach item=client from=$client}>
    <tr class="<{cycle values='even,odd'}>">
        <td class="txtcenter"><{$client.name}></td>
        <{if $client.uid == 0}>
        <td class="txtcenter"><{$client.uname}></td>
        <{else}>
        <td class="txtcenter"><a title="<{$client.uname}>" href="<{$xoops_url}>/userinfo.php?uid=<{$client.uid}>" ><{$client.uname}></a></td>
        <{/if}>
        <td class="txtcenter"><{$client.banner_active}></td>
        <td class="txtcenter"><{$client.email}></td>
        <td class="xo-actions txtcenter">
            <img onclick="display_dialog(<{$client.cid}>, true, true, 'slide', 'slide', 325, 400);" src="<{xoAdminIcons display.png}>" alt="<{$smarty.const._AM_BANNERS_VIEW}>" title="<{$smarty.const._AM_BANNERS_VIEW}>" />
            <a href="clients.php?op=edit&amp;cid=<{$client.cid}>" title="<{$smarty.const._AM_BANNERS_EDIT}>">
                <img src="<{xoAdminIcons edit.png}>" alt="<{$smarty.const._AM_BANNERS_EDIT}>" />
            </a>
            <a href="clients.php?op=delete&amp;cid=<{$client.cid}>" title="<{$smarty.const._AM_BANNERS_DELETE}>">
