...
if ('start' === $op) {
    ...
    if (0 == $totalCat) {
        ...
        if (0 == $totalArticles) {
            ...
            $form->addElement(new Hidden('from_module_version', $_POST['from_module_version']));

            $form->display();
        }
    }

    Publisher\Utils::closeCollapsableBar('xfsectionimport', 'xfsectionimporticon');
    $xoops->footer();
}
...
if ('go' === $op) {
    Publisher\Utils::cpHeader();
    //publisher_adminMenu(-1, _AM_PUBLISHER_IMPORT);
    Publisher\Utils::openCollapsableBar('xfsectionimportgo', 'xfsectionimportgoicon', sprintf(_AM_PUBLISHER_IMPORT_FROM, $importFromModuleName), _AM_PUBLISHER_IMPORT_RESULT);
