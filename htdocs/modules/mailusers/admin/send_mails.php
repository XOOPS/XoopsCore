...
$memberslist_id = Request::getArray('memberslist_id', []);

// Call Header
$xoops->header('admin:mailusers/mailusers_send_mail.tpl');
// Define Stylesheet
$xoops->theme()->addBaseStylesheetAssets('modules/system/css/admin.css');
$xoops->theme()->addBaseScriptAssets(['@jquery', 'media/xoops/xoops.js', 'modules/system/js/admin.js']);
...
switch ($op) {
    ...
    default:
        ...
        //----------------------------------------
        if (!empty($memberslist_id) && ('POST' === Request::getMethod())) {
            ...
            $users_label = new Xoops\Form\Label(_AM_MAILUSERS_SENDTOUSERS2, mb_substr($display_names, 0, -2));
    ...
    case 'send':
        ...
        $mail_send_to = Request::getArray('mail_send_to', ['mail']);
        ...
        $mail_to_group = Request::getArray('mail_to_group', []);
        $mail_to_group = array_map('intval', $mail_to_group);
        $mail_start = Request::getInt('mail_start', 0);
        $mail_to_user = Request::getArray('mail_to_user', []);
