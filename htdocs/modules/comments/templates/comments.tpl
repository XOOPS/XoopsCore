<div style="text-align: center; padding: 0px; margin:0px;">
    <form class="form-inline" method="get" action="{$page_name}">
        ...
        <select class="form-control" name="com_mode">
        ...
        <div class="form-group">
        <select class="form-control" name="com_order">
        ...
        {/if}
        <button type="submit" class="btn btn-default">{$smarty.const._MD_COMMENTS_REFRESH}</button>
        {if $postcomment_link}
        <button type="button" class="btn btn-default" onclick="self.location.href='{$postcomment_link}{$link_extra}'">{$smarty.const._MD_COMMENTS_POSTCOMMENT}</button>
        {/if}
    </form>
