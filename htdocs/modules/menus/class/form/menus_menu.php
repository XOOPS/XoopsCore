...
class MenusMenus_menuForm extends XoopsThemeForm
{
    /**
     * @param MenusMenu $obj
     */
    public function __construct(MenusMenu &$obj)
    {
        ...
        $this->addElement(new XoopsFormText(_AM_MENUS_MENU_ALTTITLE, 'alt_title', 50, 255, $obj->getVar('alt_title'), ''));

        $this->addElement(new XoopsFormText(_AM_MENUS_MENU_LINK, 'link', 50, 255, $obj->getVar('link'), ''));

        $this->addElement(new XoopsFormText(_AM_MENUS_MENU_IMAGE, 'image', 50, 255, $obj->getVar('image'), ''));
        ...
        $this->addElement( new XoopsFormLabel(_AM_MENUS_MENU_PARENT, $parent_select));

        $formvis = new XoopsFormSelect(_AM_MENUS_MENU_VISIBLE, "visible", $obj->getVar('visible'));
        $formvis->addOption("0", XoopsLocale::NO);
        $formvis->addOption("1", XoopsLocale::YES);
        $this->addElement($formvis);

        $formtarget = new XoopsFormSelect(_AM_MENUS_MENU_TARGET, "target", $obj->getVar('target'));
        ...
        $formgroups = new XoopsFormSelectGroup(_AM_MENUS_MENU_GROUPS, "groups", true, $obj->getVar('groups'), 5, true);
        $formgroups->setDescription(_AM_MENUS_MENU_GROUPS_HELP);
        $this->addElement($formgroups);

        $formhooks = new XoopsFormSelect(_AM_MENUS_MENU_ACCESS_FILTER, "hooks", $obj->getVar('hooks'), 5, true);
        $accessFilter = array();
        ...
        $formcss = new XoopsFormText(_AM_MENUS_MENU_CSS, 'css', 50, 255, $obj->getVar('css'));
