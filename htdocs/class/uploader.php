...
class XoopsMediaUploader
{
    ...
    public function __construct($uploadDir, $allowedMimeTypes, $maxFileSize = 0, $maxWidth = null, $maxHeight = null)
    {
        ...
        $this->extensionToMime = include $xoops->path('include/mimetypes.inc.php');
    ...
    public function fetchMedia($media_name, $index = null)
    {
        if (empty($this->extensionToMime)) {
            $this->setErrors(XoopsLocale::E_LOADING_MIME_TYPES_DEFINITION);
            return false;
        }
    ...
    public function getErrors($ashtml = true)
    {
        if (!$ashtml) {
            ...
            $ret = '';
            if (count($this->errors) > 0) {
                $ret = '<h4>' . sprintf(XoopsLocale::EF_ERRORS_RETURNED_WHILE_UPLOADING_FILE, $this->mediaName) . '</h4>';
