...
if (!empty($editor)) {
    ...
    $sample_form->addElement(new XoopsFormEditor('Your message', $options['name'], $options, $nohtml = false, $onfailure = 'textarea'), true);
    $sample_form->addElement(new XoopsFormText('SOME REQUIRED ELEMENTS', 'required_element2', 50, 255, $required_element2), true);
    $sample_form->addElement(new XoopsFormButton('', 'save', XoopsLocale::A_SUBMIT, 'submit'));
    $sample_form->display();
}