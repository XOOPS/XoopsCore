...
class Factory
{
    ...
    public function create($name, $dirname = null, $optional = false)
    {
        ...
        $foundColon = mb_strpos($name, ':');
        if (false !== $foundColon) {
            $scheme = mb_substr($name, 0, $foundColon);
            $baseName = mb_substr($name, $foundColon + 1);
    ...
    {
        $this->schemes[mb_strtolower($name)] = $className;
    }
